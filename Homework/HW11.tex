...
\setcounter{enumi}{41}
	\item Find the limit. Use l'Hospital's Rule where appropriate. If there is a more elementary method, consider using it. If l'Hospital's rule does not apply, explain why.
		$$\underset{x\rightarrow \infty}{\text{lim}}(1+\frac{a}{x})^{\frac{b}{x}}$$
\setcounter{enumi}{53}
...
\begin{enumerate}
	\item $\underset{x\rightarrow \frac{\pi}{2}}{\text{lim}}\frac{\sin(x)-1}{x}$ Not indeterminate
	\item $\underset{x\rightarrow \infty}{\text{lim}}(1+\frac{2}{x})^{3x}$ 
	\begin{itemize}
		\item $\ln(L)=3x\ln(1+\frac{2}{x})$
		\item $\ln(L)=3\infty\ln(1+\frac{2}{\infty})$
		\item $\ln(L)=3x\ln(1)$
		\item $\ln(L)=\infty \cdot 0$
		\item $0\cdot \infty$, so it is an indeterminate form.
	\end{itemize}
	\item $\underset{x\rightarrow 0^+}{\text{lim}}x^{\sin(x)}$
\end{enumerate}
